...
public static class VoronoiUtil3D
{
    public const int MAP_XYZ = 32;
    public const int VORONOI_COUNT_MAX = 1;

    // 点の座標と渡される座標の差分の二乗を返す
    public static int DistanceSqrt_D3D(Point3D point3D, int x, int y, int z)
    {
        int xd = x - point3D.x;
        int yd = y - point3D.y;
        int zd = z - point3D.z;
...
public class Voronoi3D
{
    ...
    {
        int mapX = VoronoiUtil3D.MAP_XYZ - 20;
        int mapY = VoronoiUtil3D.MAP_XYZ - 20;
        int mapZ = VoronoiUtil3D.MAP_XYZ - 20;
    ...
    void CreateSites3D()
    {
        int mapX = VoronoiUtil3D.MAP_XYZ, mapY = VoronoiUtil3D.MAP_XYZ, mapZ = VoronoiUtil3D.MAP_XYZ, d, ind, dmin;
        for (int dd = 0; dd < mapZ; dd++)
        {
            for (int hh = 0; hh < mapY; hh++)
            {
                for (int ww = 0; ww < mapX; ww++)
                {
                    ...
                    {
                        Point3D p = _points[it];
                        d = VoronoiUtil3D.DistanceSqrt_D3D(p, ww, hh, dd);
                        if (d < dmin) // 
                        {
                            dmin = d; // 一番近い母点との距離
                            ind = it; // 一番近い母点の添え字
